...
def run(args):
    ...
    elif 'vit' in args.arch_name:
        net = timm.create_model(args.arch_name, pretrained=True, num_classes=1000)
    elif 'swin' in args.arch_name:
        net = timm.create_model(args.arch_name, pretrained=True, num_classes=1000)
    elif 'deit' in args.arch_name:
        net = timm.create_model(args.arch_name, pretrained=True, num_classes=1000)
    else:
    ...
    DeepInversionEngine = DeepInversionClass(args=args, net_teacher=net,
    ...
    DeepInversionEngine.generate_batch(args, net_student=net_student)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-s', '--worldsize', type=int, default=1, help='Number of processes participating in the job.')
    parser.add_argument('--gpu_id', '--gpu', type=str, default='0', help='set the gpu id to be used')
    ...
    parser.add_argument('--iter_num', default=2000, type=int, help='iter_num')
    ...
    parser.add_argument('--beta', type=float, default=1, help='beta to control the sharpness of images')
    ...
    parser.add_argument('--new_tv', action='store_true', help='whether to use new tv loss')
