...
def end_to_end_finetune(train_loader, metric_loader, test_loader, model, t_model, max_norm, LOG, args):
    # Data loading code
    end = time.time()
        ...
        if 'swin' in args.model:
            if args.update_setting == 'before': # freeze blocks after the last neighbour block
                for block_idx in range(model.depth):
                    ...
                    if (layer_id == model.last_neighbour_block[0] and block_id > model.last_neighbour_block[1]) or layer_id > model.last_neighbour_block[0]:
                        LOG.info(f'freeze block {(layer_id, block_id)}.')
                        for param in model.layers[layer_id].blocks[block_id].parameters():
                            param.requires_grad = False
        else:
            if args.update_setting == 'before': # freeze blocks after the last neighbour block
                for i in range(model.last_neighbour_block+1, len(model.blocks)):
                ...
                for i in range(model.first_neighbour_block):
                    LOG.info(f'freeze block {i}.')
                    for param in model.blocks[i].parameters():
                        param.requires_grad = False
                for i in range(model.last_neighbour_block+1, len(model.blocks)):
    ...
    model.train()
    model.get_feat = args.get_feat
    t_model.eval()
    t_model.get_feat = args.get_feat
    ...
    for iter_nums in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            # print(args.gpu, iter_nums, len(data))
        ...
        #     validate(test_loader, model, LOG)
        #     model.train()
        #     model.get_feat = args.get_feat
            
        if (iter_nums+1) % args.print_freq == 0:
            msg = "Epoch:{}| Time {batch_time.val:.3f} ({batch_time.avg:.3f}) | Data {data_time.val:.3f} ({data_time.avg:.3f}) | Loss {loss:.4f}".format(
                iter_nums+1,
        ...
        if (iter_nums+1) % args.eval_freq == 0:
            metric_loss = metric(metric_loader, model, t_model, LOG, args)
            val_top1, val_top5 = validate(test_loader, model, LOG)
            model.train()
            model.get_feat = args.get_feat
    ...
    return metric_loss, val_top1, val_top5
...
def metric(metric_loader, model, origin_model, LOG, args):
    def metric_ln(model):
        attn_ln_weight = dict()
        attn_ln_bias = dict()
        ffn_ln_weight = dict()
    ...
    origin_model.cuda()
    origin_model.eval()
    origin_model.get_feat = args.get_feat
    model.cuda()
    model.eval()
    model.get_feat = args.get_feat
